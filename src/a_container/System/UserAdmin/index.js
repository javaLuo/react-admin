...
import { Form, Button, Icon, Input, Table, message, Popconfirm, Modal, Radio, Tooltip, Divider, Select } from 'antd';
import './index.scss';
...
export default class RoleAdminContainer extends React.Component {
  ...
  onOk() {
    ...
    form.validateFields(['formUsername', 'formPassword', 'formPhone', 'formEmail', 'formDesc', 'formConditions'], (err, values) => {
      if (err) {
        return false;
      }
      ...
      <div>
        <div className="g-search">
          <ul className="search-func">
            <li>
              <Button type="primary" disabled={!p.includes('user:add')} onClick={() => this.onModalShow(null, 'add')}>
          ...
          {p.includes('user:query') && (
            <ul className="search-ul">
              <li>
                <Input placeholder="请输入用户名" onChange={e => this.searchUsernameChange(e)} value={this.state.searchUsername} />
              </li>
              <li>
              ...
              <li>
                <Button icon="search" type="primary" onClick={() => this.onSearch()}>
                  搜索
                </Button>
              </li>
            </ul>
        ...
        {/* 新增&修改&查看 模态框 */}
        <Modal title={{ add: '新增', up: '修改', see: '查看' }[this.state.operateType]} visible={this.state.modalShow} onOk={() => this.onOk()} onCancel={() => this.onClose()} confirmLoading={this.state.modalLoading}>
          <Form>
            <FormItem label="用户名" {...formItemLayout}>
              {getFieldDecorator('formUsername', {
                initialValue: undefined,
                rules: [{ required: true, whitespace: true, message: '必填' }, { max: 12, message: '最多输入12位字符' }],
              })(<Input placeholder="请输入用户名" disabled={this.state.operateType === 'see'} />)}
            </FormItem>
            <FormItem label="密码" {...formItemLayout}>
              {getFieldDecorator('formPassword', {
                initialValue: undefined,
                rules: [{ required: true, whitespace: true, message: '必填' }, { min: 6, message: '最少输入6位字符' }, { max: 18, message: '最多输入18位字符' }],
        ...
        <RoleTree title={'分配角色'} data={this.state.roleData} visible={this.state.roleTreeShow} defaultKeys={this.state.roleTreeDefault} loading={this.state.roleTreeLoading} onOk={v => this.onRoleOk(v)} onClose={() => this.onRoleClose()} />
      </div>
    );
  }
}
